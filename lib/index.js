...
customElements.define('nostr-event', class extends HTMLElement {
  connectedCallback () {
    const event = this.getAttribute('event') || '' // grab the event attribute and fallback to empty string if not present
    const content = this.getAttribute('content') || '' // grab the content attribute and fallback to empty string if not present
    if (event || content) { // if event or content is present
      render(html`<${Event} event=${event} content=${content}/>`, this)
    }
  }
})
